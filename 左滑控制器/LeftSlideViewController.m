...
@interface LeftSlideViewController ()<UIGestureRecognizerDelegate,PushLeftItemDelegete>{
 ...
 */
- (instancetype)initWithLeftView:(LeftSortsViewController *)leftVC andMainView:(BaseTabBarController *)baseTabBarVC{
    if(self = [super init]){
        ...
        //注册代理， 要响应回调push item 选项的界面， 必须代理
        leftVC.delegate = self;
        
        [self.baseTabBarVC.view addGestureRecognizer:self.pan];
        ...
        [self.view addSubview:self.baseTabBarVC.view];
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (((self.baseTabBarVC.view.x <= 0) && (_scalef <= 0)) || ((self.baseTabBarVC.view.x >= (kScreenWidth - kMainPageDistance )) && (_scalef >= 0))){
        //边界值管控
        _scalef = 0;
        needMoveWithTap = NO;
    }
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance))){
        ...
        //超出范围，
        if (self.baseTabBarVC.view.x < 0)
        {
            [self closeLeftView];
            _scalef = 0;
        }
        else if (self.baseTabBarVC.view.x > (kScreenWidth - kMainPageDistance))
        {
            [self openLeftView];
            _scalef = 0;
        }
...
- (void)closeLeftView{
    ...
    [UIView beginAnimations:nil context:nil];
    self.baseTabBarVC.view.transform = CGAffineTransformScale(CGAffineTransformIdentity,1.0,1.0);
    self.baseTabBarVC.view.center = CGPointMake(kScreenWidth / 2, kScreenHeight / 2);
    self.closed = YES;
...
{
    [UIView beginAnimations:nil context:nil];
    self.baseTabBarVC.view.transform = CGAffineTransformScale(CGAffineTransformIdentity,kMainPageScale,kMainPageScale);
    self.baseTabBarVC.view.center = kMainPageCenter;
    self.closed = NO;
...
- (void)disableTapButton{
    ...
    for (UIButton *tempButton in [_baseTabBarVC.navigationController.navigationBar subviews]){
        if ([tempButton isKindOfClass:[UIBarButtonItem class]]) {
    ...
    if (!self.sideslipTapGes){
        ...
        [self.baseTabBarVC.view addGestureRecognizer:self.sideslipTapGes];
...
- (void) removeSingleTap{
    ...
    for (UIButton *tempButton in [self.baseTabBarVC.navigationController.navigationBar subviews]){
        if ([tempButton isKindOfClass:[UIBarButtonItem class]]) {
            [tempButton setUserInteractionEnabled:YES];
        }
    }
    [self.baseTabBarVC.view removeGestureRecognizer:self.sideslipTapGes];
...
//点击左侧视图 tableView 选线的回调
-(void)pushLeftItemVC:(LeftItemBaseVC *)leftItemVC{
    
    [self.baseTabBarVC.selectedViewController pushViewController:leftItemVC animated:YES];
}
